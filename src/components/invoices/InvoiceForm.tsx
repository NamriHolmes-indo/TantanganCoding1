...
const TambahInvoice = ({ onActionComplete }: { onActionComplete: (status: string, missingFields?: string[]) => void }) => {
    ...
    const handleAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setAmount(Number(formattedValue.replace(/[^\d.-]/g, ''))); // Ensure it's a number
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const newInvoice = {
            id: generateInvoiceNumber(), // Example ID, adjust as needed
            name,
            number,
            dueDate,
            amount,
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col text-black p-6 gap-5">
            ...
            <section className="barisInput flex w-full gap-10 self-center justify-between">
                ...
                <div className="flex w-full max-w-[48%] flex-col gap-2 self-center justify-start">
                    ...
                    <div className="flex items-center bg-[#E2E8F0] border border-solid border-gray-300 rounded-md">
                        <span className="bg-[#D9D9D9] px-3 py-2 text-gray-700 mr-2 rounded-md">RP</span>
                        <input
                            type="text"
                            name="amount"
                            id="amount"
